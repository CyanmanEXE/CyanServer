...
---create component to telegraph moves
---@param character any
---@param pattern [] array of tiles
---@return any warning component
function battle_helpers.create_warning_component(character, pattern)
    local warning_component = Battle.Component.new(character, Lifetimes.Battlestep)
    warning_component.update_func = function(self)
        for index, tile in ipairs(pattern) do
            tile:highlight(Highlight.Flash)
        end
    end
    character:register_component(warning_component)
