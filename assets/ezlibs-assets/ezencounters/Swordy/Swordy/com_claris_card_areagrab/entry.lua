...
chip.card_create_action = function(actor, props)
	...
	action.execute_func = function(self, user)
		...
		for i = count, max, 1 do
			...
			check_up = tile_up and user:get_team() ~= tile_up:get_team() and not tile_up:is_edge() and
				tile_up:get_team() ~= Team.Other and user:is_team(tile_up:get_tile(Direction.reverse(dir), 1):get_team())
			check_down = tile_down and user:get_team() ~= tile_down:get_team() and not tile_down:is_edge() and
				tile_down:get_team() ~= Team.Other and user:is_team(tile_down:get_tile(Direction.reverse(dir), 1):get_team())
